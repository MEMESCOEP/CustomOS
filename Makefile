...
run: $(KERNEL_IMG) f32.disk
	...
	#cp os.iso ./isofiles/boot/
	-rm os.zip
	zip * -o os.zip
	qemu-system-i386 -boot d -cdrom os.iso -drive file=f32.disk,format=raw -m size=2g -cpu phenom -soundhw pcspk

run-debug: $(KERNEL_IMG) f32.disk
	@echo "gdb target localhost:1234"
	qemu-system-i386 --kernel $(KERNEL_IMG) -drive file=f32.disk,format=raw -m size=2048 -S -s
...
f32.disk:
	-rm f32.disk
	dd if=/dev/zero of=f32.disk bs=1M count=256
...
$(KERNEL_IMG) : $(OBJECTS) linker.ld
	$(CC) $(LDFLAGS) -T linker.ld -o kernel.bin $(OBJECTS)
